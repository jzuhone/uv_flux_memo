...
Filters (OBFs) facing the spacecraft interior. The contamination layer continues to accumulate even after 16 years on orbit. The accumulation rate, the chemical composition, and the spatial distribution of the contaminant have all varied with time over the mission. In this paper, we discuss our current understanding of the time-variable accumulation rate, chemical composition and spatial distribution.
...
photons at longer wavelengths (Figure \ref{fig:bright_earth_intensity}).

We can now make a rough estimate of the amount of contaminant that could be polymerized by these photons. Since we
do not know the molecular composition of the contaminant, it will be necessary to make some assumptions. We will assume
that the contaminant is composed primarily of H and C atoms, with some amounts of O and F, and that the primary molecular form of the contaminant is that of a hydrocarbon with $N_{\rm C}$ C atoms per molecule. This implies that the number of bonds per molecule are:

\begin{eqnarray}
N_{\rm C=C} &\approx& N_{\rm C} - 1 \\
N_{\rm H-C} &\approx& 2N_{\rm C} + 2
\end{eqnarray}

Assuming 100\% efficiency (e.g., that each UV photon which impinges upon the OBFs breaks a bond), we can estimate
the number of broken bonds per square area as:


Since we are concerned with breaking H-C and C=C bonds,
