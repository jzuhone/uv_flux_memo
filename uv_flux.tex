...
polymerized material is then given by $N_{\rm C} = H_{\rm C-C}n_{\rm C}$, where $n_{\rm C}$ is the number of C atoms per
molecule. For $n_{\rm C} = 17$, $N_{\rm C} = 5.75 \times 10^{16}$~cm$^{-2}$, which is a small fraction of the areal density
of the contaminant present on the OBFs (Figure \ref{fig:areal_density}).
...
as above, we find $N_{\rm C} = 1.55 \times 10^{17}$~cm$^{-2}$, slightly higher than the predicted amount resulting from
the bright Earth, but still far smaller than the actual areal density of the contaminant.
...
over the lifetime of the mission. We have determined that there has not been a sufficient UV fluence to polymerize
a significant amount of the contaminant. The following limitations of our study must be kept in mind:
